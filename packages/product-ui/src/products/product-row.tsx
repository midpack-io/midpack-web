...
import { BundleStepper } from "../ds/bundle-stepper";
import { useProductNav } from "../context/nav-context";
...
type ProductRowProps = {
  ...
  // When false (e.g. the marketing hero), the row is a pure display: the
  // stepper still animates on hover, but the status/person/unlock dropdowns
  // and the head's inline editors are disabled, so the whole row is a clean
  // click-through. Defaults to true (full app behavior).
  interactive?: boolean;
};
...
export function ProductRow({
  ...
  const navigate = () => openProduct(href);
  ...
  return (
    ...
    >
      ...
      <BundleStepper
        ...
        onPerformerChange={interactive ? handlePerformerChange : undefined}
        onUnlock={interactive ? handleUnlock : undefined}
