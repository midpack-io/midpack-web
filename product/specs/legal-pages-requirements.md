# Legal Pages Requirements — iterate.guru
...
| Contact email | `gaponenko.mm@gmail.com` (or `legal@iterate.guru` if the alias is wired before launch — designer's call which one to render) |
...
- **Product name:** iterate.guru (lowercase wordmark, same as `email-requirements.md`).
...
|---|---|---|
| `/terms` | `Terms of Service — iterate.guru` | "Plain-English terms for using iterate.guru, our research preview." |
| `/privacy` | `Privacy Notice — iterate.guru` | "What we collect, why, and your rights — including GDPR rights for EU users." |
...
- One-paragraph TL;DR styled as a callout / note block (subtle border, muted background — designer's pattern):
  ...
  > **In short.** iterate.guru is a research preview built by one person. You can use it for free, but it has no SLA, no warranty, and your data may be removed with notice. Don't upload anything illegal or anything you'd be unhappy losing. By using the service you agree to the terms below.
...
- Body (this is the section that distinguishes iterate.guru from a generic SaaS):
...
- TL;DR callout:
  ...
  > **In short.** We collect your Google profile (name, email, picture), the files and feedback you upload, and basic logs for security and debugging. We use this to run iterate.guru — nothing more. We share data only with the platforms we run on (listed below). We don't sell your data, we don't use it to train AI models, and you can delete your account at any time. **EU/EEA users:** you have the rights GDPR gives you, and we'll honor them.
...
- Body:
  ...
  > The data controller for iterate.guru is **Maksym Haponenko**, an individual based in Ukraine. You can reach me at **[gaponenko.mm@gmail.com](mailto:gaponenko.mm@gmail.com)** for any privacy question, complaint, or rights request.
  >
...
```
© 2026 iterate.guru — research preview by Maksym Haponenko    Terms · Privacy · Feedback
```
...
- **Dedicated "Children" section in the Privacy Notice** — iterate.guru is a productivity tool, not a child-directed service. The Terms include a one-line note for under-18s + a parent/guardian removal path; that's the whole posture.
...
4. **Contact email — `gaponenko.mm@gmail.com` vs. `legal@iterate.guru`?** A custom alias on the iterate.guru domain reads more serious and survives the founder's personal Gmail being deprecated. Recommendation: **wire up `legal@iterate.guru` and `privacy@iterate.guru` aliases that forward to Gmail** before launch, render those on the page. Cost: 5 minutes of DNS / Resend inbound config.
