...
- **Fixed-height interactive containers need `leading-none`.** Anything with an explicit height (filter chip triggers at `h-[28px]`, badges, button-style controls) inherits `body { line-height: 1.45 }` from `globals.css`, which inflates the line-box past the glyph height. `items-center` then centers that inflated box, not the visible text, so the glyph drifts upward inside the container. Add `leading-none` to the trigger so the line-box collapses to font-size; descenders sit safely in the remaining vertical padding. This is the *internal* counterpart to the baseline rule above: `items-baseline` handles chip↔prose alignment *between* elements; `leading-none` handles centering *within* a chip's own box.
...
This app is `apps/app` in the **midpack-web Turborepo** (customer platform). It runs on port **3002**
behind the dev proxy at `http://app.localhost:3000`. Run commands from the repo root unless noted.

- `pnpm dev` (repo root) — runs all three apps + the proxy. This app is at http://app.localhost:3000.
- `pnpm dev --filter app` — just this app (still reachable through the proxy, or directly on :3002).
- `pnpm build` / `pnpm typecheck` — via Turborepo. MSW is enabled when `NEXT_PUBLIC_API_MOCKING=enabled`.

Lint is intentionally not configured for the stage-1 prototype; rely on TypeScript strict.

## Auth (Turborepo packages)

Auth is wired via the shared workspace packages (not in this app): `@midpack/auth` (`useAuth`,
`ProtectedRoute`, `PublicRoute`), `@midpack/api-client` (the `/api/v1/auth/*` client + SSO cookie),
and `@midpack/ui` (the shared auth views). The product shell under `src/app/(app)/` is wrapped in
`<ProtectedRoute>`; the public auth pages live in `src/app/(auth)/` plus top-level `reset-password` /
`verify-email`. Set `NEXT_PUBLIC_API_URL=http://localhost:8000` for the real backend, or leave it empty
to mock auth via the MSW handlers in `src/mocks/handlers/auth.ts`. This is the network seam too: auth
mocks mirror the real contract (see `handoffs/auth-frontend-handoff.md` in the umbrella).
